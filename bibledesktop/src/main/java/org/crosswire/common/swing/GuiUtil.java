...
public final class GuiUtil {
    ...
    public static void restrainedPack(Window win, float maxx, float maxy) {
        ...
        // If the window is wider than the screen, clip it
        if (win.getSize().width > (screenDim.width * maxx)) {
            win.setSize((int) (screenDim.width * maxx), win.getSize().height);
        }

        // If the window is taller than the screen, clip it
        if (win.getSize().height > (screenDim.height * maxy)) {
            win.setSize(win.getSize().width, (int) (screenDim.height * maxy));
        }
